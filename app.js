...
function tooltipContent(feature) {
  return infoGridHtml(getFeatureInfo(feature), 'tooltip-title');
}

function updateHoverInfo(feature) {
  if (!els.hoverInfo) return;
  els.hoverInfo.classList.remove('hover-empty');
  els.hoverInfo.innerHTML = infoGridHtml(getFeatureInfo(feature), 'hover-title');
}

function resetHoverInfo() {
  if (!els.hoverInfo) return;
  els.hoverInfo.classList.add('hover-empty');
  els.hoverInfo.textContent = 'Pasa el cursor sobre un polígono.';
}
...
function renderSectionLabels() {
  ...
  if (!state.showSectionLabels || !state.geojsonData) {
  ...
  state.geojsonData.features.forEach((feature) => {
    ...
    const marker = L.marker(getLabelLatLng(feature), {
      interactive: false,
      keyboard: false,
      icon: L.divIcon({
...
function refreshLayerStyles() {
  ...
  state.geojsonLayer.eachLayer((layer) => {
    ...
    if (layer.getTooltip()) layer.setTooltipContent(tooltipContent(layer.feature));
  });
  renderSectionLabels();
  resetHoverInfo();
  updateMetrics();
