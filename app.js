...
function initMap() {
  ...
  state.map.on('click', (event) => {
    if (!event.originalEvent._sectionClick) clearSelection();
  });
}
...
function getVueltaColor(view) {
  ...
  // Color automático para vueltas mayores a las configuradas.
  // Mantiene tonos profesionales sin repetir exactamente el mismo color.
  const hue = (n * 47) % 360;
...
function onEachFeature(feature, layer) {
  layer.on('mouseover', () => {
    ...
    layer.setStyle({ weight: 2.2, color: CONFIG.COLORS.bordeHover, opacity: 1 });
  ...
  layer.on('click', (event) => {
    if (event.originalEvent) event.originalEvent._sectionClick = true;
    selectLayer(layer);
  });
}
  ...
  state.geojsonData.features.forEach((feature) => {
    ...
    const marker = L.marker(latLng, {
      ...
      icon: L.divIcon({
        ...
        html: `<span>${escapeHtml(feature.properties.SECCION)}</span>`,
        iconSize: [42, 18],
        iconAnchor: [21, 9]
      })
    });
...
async function main() {
  try {
    ...
    updateMetrics();
    updateLegend();
    setStatus('Mapa listo. Pasa el cursor sobre un polígono o selecciónalo para ver su tarjeta lateral.');
  } catch (error) {
    console.error(error);
