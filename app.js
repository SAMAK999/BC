...
function getRowForFeature(feature) {
  const seccion = normalizeKey(feature.properties.SECCION);
  return state.sheetRowsBySection.get(seccion) || null;
}
...
function getStyle(feature) {
  ...
  return {
    color: CONFIG.COLORS.borde,
    weight: visible ? 0.65 : 0.25,
    opacity: visible ? 0.7 : 0.12,
    fillColor,
...
function popupContent(feature) {
  ...
  const status = getStatus(feature);
  const selected = state.selectedView;
  const rawValue = row ? (row[selected] ?? '') : 'Sin coincidencia en CSV';
  const vueltas = row ? (row.VUELTAS_RECORRIDAS ?? '') : '';
  const ultima = row ? (row.ULTIMA_VUELTA ?? '') : '';

  return `
    <div class="popup-title">Sección ${props.SECCION}</div>
    <div class="popup-grid">
      <span>Municipio</span><strong>${props.MUNICIPIO || ''}</strong>
      <span>Distrito local</span><strong>${props.DL || ''}</strong>
      <span>Distrito federal</span><strong>${props.DF || ''}</strong>
      <span>Vista</span><strong>${selected}</strong>
      <span>Estatus</span><strong>${status}</strong>
      <span>Valor</span><strong>${rawValue}</strong>
      <span>Vueltas recorridas</span><strong>${vueltas}</strong>
      <span>Última vuelta</span><strong>${ultima}</strong>
    </div>
  `;
}
...
function onEachFeature(feature, layer) {
  ...
  layer.bindTooltip(String(props.SECCION), {
    sticky: true,
    direction: 'top',
...
function renderGeoJSON() {
  ...
  state.map.fitBounds(state.geojsonLayer.getBounds(), { padding: [18, 18] });
}
